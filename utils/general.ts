...
export const isAddressKey = (address: string) =>
  address?.length >= 14 && address?.length <= 100;
export const isAddressId = (address: string) =>
  address?.length > 4 && address?.length <= 13;
...
export const normalizeVerifiers = (verifiers: any[]) =>
  verifiers.map((verifier) => normalizeVerifier(verifier));

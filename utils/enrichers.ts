	...
	verifiers.map((verifier) => ({
		...
		fromInterplanetaryOne: {
			...verifiersFromInterplanetaryOne.find(
				(fromIpo) =>
					verifier.addressId === fromIpo.addressId || verifier.addressKey === fromIpo.address,
			),
		},
	}));
...
export const enrichWithVerifiedClients = (
	...
	verifiers.map((verifier) => ({
		...
		verifiedClientsFromInterplanetaryOne:
			_.groupBy(verifiedClientsFromInterplanetaryOne ?? [], (vc) =>
				vc.verifierAddressId === verifier.addressId,
			).true ?? [],
	}));
